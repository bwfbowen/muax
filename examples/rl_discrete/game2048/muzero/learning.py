...
from muzero import networks as mz_networks
...
from muax.frameworks.acme.jax.muzero import RAW_VALUES_KEY, POLICY_PROBS_KEY
import tree
...
class TrainingState(NamedTuple):
    params: mz_networks.MZNetworkParams
...
class MZLearner(acme.Learner):
    ...
    def __init__(
        ...
        network_params = mz_networks.init_params(
            self._networks,
            environment_spec,
            key,
            add_batch_dim=True,
    ...
    def _gradient_step(
        ...
        step = state.step + 1
        new_params = mz_networks.MZNetworkParams(
    ...
    def _loss_fn(
        ...
        def unroll_func(i, loss_s):
            ...
            s = mz_utils.scale_gradient(s, 0.5)
            r, ns = self._networks.dynamic_network.apply(params.dynamic, s, batch.action[:, i - 1])
            v, logits = self._networks.prediction_network.apply(params.prediction, ns)
    ...
    def _compute_value_target(self, batch: adders.Step) -> jnp.ndarray:
        ...
        n_step_return_fn = functools.partial(mz_utils.n_step_bootstrapped_returns, 
    ...
    def step(self):
        ...
        with jax.profiler.StepTraceAnnotation('step', step_num=self._current_step):
            ...
            self._current_step, metrics = mz_utils.get_from_first_device(
    ...
    def get_variables(self, names: List[str]) -> network_lib.Params:
        params = mz_utils.get_from_first_device(self._state.params)
        variables = {
            'policy': params,
        }
    ...
    def save(self) -> TrainingState:
        return jax.tree_map(mz_utils.get_from_first_device, self._state)
