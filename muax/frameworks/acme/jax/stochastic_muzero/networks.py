...
def select_closest_code(encoder_output: jnp.ndarray) -> jnp.ndarray:
    """
    Select the closest code from the encoder output.
    
    Args:
        encoder_output: The output from the encoder network.
    
    Returns:
        A one-hot encoded vector representing the closest code.
    """
    closest_code_index = jnp.argmax(encoder_output, axis=-1)
    return nn.one_hot(closest_code_index, encoder_output.shape[-1])

class SMZNetworkParams(NamedTuple):
    ...
    *,
    ...
    class Representation(hk.Module):
        ...
        def __call__(self, obs):
            ...
            s = smz_utils.min_max_normalize(s)
            return s 
